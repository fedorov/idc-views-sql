WITH
  ...
  FROM
    `bigquery-public-data.idc_v{idc_version}.auxiliary_metadata` AS aux
  INNER JOIN
  ...
  FROM
    pre_dicom_all
  INNER JOIN
    `bigquery-public-data.idc_v{idc_version}.original_collections_metadata` AS data_collections
  ON
